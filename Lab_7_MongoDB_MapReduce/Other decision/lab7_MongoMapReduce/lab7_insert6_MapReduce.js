...
var mapOrders1 = function() {
	...
	for(var i in this.order){
		...
		values.order.push({user_id: this.user_id,order_id: this.id_k,time: this.time,amount:this.order[i].amount})
		emit(key, values);
	}
};
var reduce1 = function(key, values) {
    ...
    values.forEach(function(value) {
		...
		if("order.order_id" in value){
			result.order.push(value.order);
		}
    });
    return result;
};
db.orders.mapReduce(mapOrders1, reduce1, {"out": {"reduce": "items_orders"}});
db.items.mapReduce(mapItems1, reduce1, {"out": {"reduce": "items_orders"}});
db.items_orders.find().pretty();
db.items_orders.remove({});
/////////////////////////////////////////
...
var mapOrders1 = function() {
	...
	for(var i in this.order){
		...
		values.set[0].amount = this.order[i].amount;
		emit(key, values);
	}
};
var reduce1 = function(key, values) {
    var result = {set:[]}, orderFields = {
        "order_id": '', 
		"time": '', 
		"user_id": '', 
		"amount": ''
    ...
    values.forEach(function(value) {
		...
		if("set[0].order_id" in value){
			for (var f in value.set) {
				for (var field in value.set[f]) {
					if ((value.set[f].hasOwnProperty(field)) && !(field in orderFields)) {
						result.set[f][field] = value.set[f][field];
					}
				}
			}
		}
