...
sh.shardCollection('laba75.items', { id_t: "hashed" } )
...
sh.shardCollection('laba75.orders', { id_k: "hashed" } )
...
var reduce1 = function(key, values) {
    ...
    values.forEach(function(value) {
        if ("f_users" in value){
			...
			for(var i in value.f_users){
				result.f_users.push(value.f_users[i]);
			}
		} 
        else if ("time" in value){
			result.f_users.push(value);
        }
		else {
			for (var field in value) {
				result[field] = value[field];
...
var mapUsers_Orders2 = function() {
	...
	for(var j in this.value.f_users){
		values.id_o = this.value.f_users[j].id_o;
		values.time = this.value.f_users[j].time;
		for(var i in this.value.f_users[j].order){
			values.amount = this.value.f_users[j].order[i].amount;
			key = this.value.f_users[j].order[i].item_id; 
			emit(key, values);
		}
	}
};
...
var reduce2 = function(key, values) {
    ...
    values.forEach(function(value) {
        if ("g_items" in value){
			...
			for(var i in value.g_items){
				result.g_items.push(value.g_items[i]);
			}
		} 
        else if ("user_id" in value){
			result.g_items.push(value);
        }
		else {
			for (var field in value) {
				result[field] = value[field];
...
var mapUsers_Orders_Items3 = function() {
	...
	for(var i in this.value.g_items){
		key = this.value.g_items[i].id_o;
		values.user_id = this.value.g_items[i].user_id;
		values.name = this.value.g_items[i].name;
		values.city = this.value.g_items[i].city;
		values.telephone = this.value.g_items[i].telephone;
		values.email = this.value.g_items[i].email;
		values.time = this.value.g_items[i].time;
		values.amount = this.value.g_items[i].amount;
		emit(key, values);
	}
};
...
var reduce3 = function(key, values) {
    var result = {"h_orders":[]};
    values.forEach(function(value) {
        if ("h_orders" in value){
			for (var field in value) {
				if(field != "h_orders"){
					result[field] = value[field];
				}
			}
			for(var i in value.h_orders){
				result.h_orders.push(value.h_orders[i]);
			}
		}
		else {
			var last, temp = true;
			for(var i in result.h_orders){
				if(result.h_orders[i].user_id==value.user_id){
					result.h_orders[i].order.push({item_id:value.item_id,amount:value.amount,index:value.index,category:value.category,brand:value.brand,model:value:model,price:value.price});
					temp = false;
				}
				last = i;
			}
			if(temp){
				result.h_orders.push({user_id:value.user_id,name:value.name,city:value.city,telephone:value.telephone,email:value.email,order:[]});
				temp = true;
				for(var i in result.h_orders){
					if(result.h_orders[i].user_id==value.user_id){
						result.h_orders[i].order.push({item_id:value.item_id,amount:value.amount,index:value.index,category:value.category,brand:value.brand,model:value:model,price:value.price});
...
var mapUsers_Orders_Items5 = function() {
    var key = {
		user_id:0,
		name:'',
		year: 0}, 
	...
	for(var i in this.value.g_items){
		key.user_id = this.value.g_items[i].user_id;
		key.name = this.value.g_items[i].name;
		key.year = this.value.g_items[i].time.getFullYear();
		values.amount = this.value.g_items[i].amount;
		emit(key, values);
	}
};
var reduce5 = function(key, values) {
	var res = {result:0}, temp;
    values.forEach(function(value) {
		if(typeof(value.price)!=undefined){
			temp = value.amount*value.price;
		}
		else {
			temp = value;
		}
...
db.users_orders_items.mapReduce(mapUsers_Orders_Items5, reduce5, {"out": {"reduce": "cust_each_year"}});
db.cust_each_year.find().pretty()

var mapUsers_Orders_Items6 = function() {
    var key = {
		user_id:this._id.user_id,
		name:this._id.name}, 
	values = {
		year: this._id.year,
...
var reduce6 = function(key, values) {
	var res = {y2016:0,y2017:0,diff:0};
    values.forEach(function(value) {
		if(value.year == 2016){
			res.y2016 = value.result;
		}
		if(value.year == 2017){
			res.y2017 = value.result;
		}
    });
...
db.cust_each_year.mapReduce(mapUsers_Orders_Items6, reduce6, {"out": {"reduce": "diference"}});
db.diference.find().pretty();
/*db.diference.remove({});*/


var mapUsers_Orders_Items7 = function() {
    var key = {
		user_id:0,
		name:'',
		year: 0}, 
...
var reduce7 = function(key, values) {
	var res = {result:[]}, temp = false;
    values.forEach(function(value) {
		for(var i in res.result){
			if(value.item_id == res.result[i].item_id){
				res.result[i].amount+=value.amount;
				temp = true;
			}
		}
		if(!temp){
			res.result.push(value);
		}
		temp = false;
    });
    return res;
...
var finalize7 = function (key,result1){
	
	var res = {result:[]};
	var temp_lenght = result1.result.length;
	var temp=[], k;
	var temp_obj = {};
	for(var i = 0; i < temp_lenght; i++){
		temp.push(true);
	}
	for(var j = 0; j < temp_lenght; j++){
		k = 0;
		while(!temp[k]){
			k++;
		}
		for(var i = k; i < temp_lenght; i++){
			if(temp[i]){
				if(result1.result[k].amount < result1.result[i].amount){
					k = i;
				}
			}
		}
		temp[k] = false;
		temp_obj = result1.result[k];
		/*temp_obj.amount = result1.result[k].amount/2;*/
		res.result.push(temp_obj);
	}
	return res;
};
db.users_orders_items.mapReduce(mapUsers_Orders_Items7, reduce7, {"out": {"reduce": "top_year"},"finalize":finalize7/**/});
db.top_year.find().pretty();
db.top_year.remove({});

db.top_year.find({"_id.year":2016}).pretty();
db.top_year.find({"_id.year":2017}).pretty();
...
var map10003 = function() {
	...
	for(var i in this.value.g_items){
		key.user_id = this.value.g_items[i].user_id;
		key.name = this.value.g_items[i].name;
		value = this.value.g_items[i].amount*this.value.price;
		emit(key, value);
	}
};
...
db.users_orders_items.mapReduce(map10003, reduce10003, {"out": "task3"});
...
db.users_orders_items.mapReduce(map10003, reduce10003, {"out": "task4",query: { date: { $gt: ISODate(" 2016-01-26T14:45:51.345Z") }}});
